<template>
  <div class="company-info" v-if="$parent.companyName && ($parent.historyBidRecord || $parent.historyWinTheBidRecord)">
    <h3>{{ $parent.companyName || "-" }}</h3>
    <p>
      <a @click="record('3')">{{ $parent.historyBidRecord }}条<el-icon>
          <ArrowRight />
        </el-icon></a>
      <span>历史招标记录</span>
    </p>
    <p>
      <a @click="record('4')">{{ $parent.historyWinTheBidRecord }}条<el-icon>
          <ArrowRight />
        </el-icon></a>
      <span>历史中标记录</span>
    </p>
