  ...
  <div class="dropdown">
    <div class="fr_c fj_c" @mouseenter="mouseenterSelect" @mouseleave="mouseleaveSelect">
      <div class="dropdown-hd fr_c" :class="{ 'dropdown-hd-active': currentIndex >= 0 }">
        ...
        <el-icon :class="{ 'dropdown-hd-up': dropdownShow }">
          <ArrowDown />
        </el-icon>
      </div>
      <div class="dropdown-content" v-show="dropdownShow">
        <div class="content scrollbar">
          <div class="content-item" v-for="(item, index) in options" :key="index" :class="{ 'content-item-active': currentIndex === index }" @click="handelCurrentItem(item, index)">
            {{ item.name }}
          </div>
        </div>
      </div>
