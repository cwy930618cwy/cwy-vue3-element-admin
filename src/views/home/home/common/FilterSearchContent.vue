<template>
    <div class="FilterKeywordRelated fr_c" v-loading.fullscreen="exportLoading" element-loading-text="正在加载中...">
        <el-popover
            ...
            title=""
            trigger="hover"
            content="谨慎使用，建议关键词数量≤100，否则系统会有宕机风险"
        >
            ...
            <el-dialog v-model="dialog.show" title="上传" width="30%" :close-on-click-modal="false" destroy-on-close>
                ...
                <div class="tip-content">
                    <el-popover
                        ...
                        title="使用提示："
                        trigger="hover"
                        content="上传模式下，excel单行检索词，检索结果大于1万条或所有行搜索结果总条数大于100万条时，整个检索将终止，建议减少搜索词，多次检索。"
                    >
                        <template #reference
                            ><img class="key-tip" src="@/assets/img/icon-success-fill.png" alt="" />
                        </template> </el-popover
                    ><span>上传模式下只支持<b>精准</b>搜索！</span>
                </div>
                <div>
                    <el-upload
                    ...
                    <a class="down-a" @click="exportTemplate">
                        导出模板.excel <el-icon><Download /></el-icon>
                    </a>
                </div>
                <template #footer>
